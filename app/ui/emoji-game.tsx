...
export function EmojiGame({ code, data, token, onRefresh, connection, onlinePlayerIds }: { code: string; data: GameData; token: string; onRefresh: () => Promise<void>; connection: "connecting" | "live" | "recovering"; onlinePlayerIds: string[] }) {
  ...
  return <main className="game-shell">
    <header className="game-top"><div className="brand-mark"><span>✦</span> Signal Scramble</div><div className="header-status"><span className={`sync-status ${connection}`}>{connection === "live" ? "● Synced" : connection === "recovering" ? "↻ Reconnecting" : "○ Connecting"}</span><span className="round-pill">Emoji Transmission · {round.number}/5</span>{data.role === "player" && <LeaveRoomButton code={code} token={token} />}</div></header>
    <section className="game-stage">
      <div className="game-card">
        ...
        {round.status === "briefing" && round.number > 1 && <><p className="eyebrow">Next transmission</p><div className="round-progress" aria-label={`Round ${round.number} of 5`}>{[1, 2, 3, 4, 5].map((step) => <i className={step <= round.number ? "active" : ""} key={step} />)}</div><h1>Signal {round.number} is ready</h1><p className="game-instructions">Take a breath and make sure everyone is watching. The 45-second timer starts when the host launches the signal.</p>{data.role === "host" ? <button className="primary-button game-action" disabled={busy} onClick={() => hostAction("begin")}>Launch signal {round.number} →</button> : <div className="waiting-chip">The next signal launches soon…</div>}</>}
